...
class GoogleFontDataset(Dataset):
    ...
    def get_pairs(self, content_english=False, style_english=False):
        ...
        if content_english == style_english:
            ...
            chars = random.sample(content_unicode_list, k=self.args.reference_imgs.char+1)
            content_char = chars[-1]
            style_chars = chars[:-1]
        else:
            content_char = random.choice(content_unicode_list)
            style_chars = random.sample(style_unicode_list, k=self.args.reference_imgs.char)
            
        fonts = random.sample(self.content_meta[lang_content].keys(), k=self.args.reference_imgs.char+1)
        content_fonts = fonts[:-1]
        style_font = fonts[-1]
    ...
    def __getitem__(self, idx):
        ...
        Args:
            idx (int): torch dataset에서 부여하는 index 값

        Returns:
            dict: 아래의 key를 가진 element를 return
...
if __name__ == '__main__':
    ...
    metadata_path = "/home/hksong/google_font_metatools/_google_ofl_metadata.json"
    _dataset = GoogleFontDataset(hp, metadata_path, FONT_DIR)
    TEST_ITER_NUM = 4
